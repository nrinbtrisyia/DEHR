...
$sqlmain = "SELECT schedule.scheduleid, schedule.title, doctor.fname AS fname, schedule.scheduledate, schedule.scheduletime, schedule.nop 
FROM schedule 
...
ORDER BY schedule.scheduledate DESC"; 
                                                ...
                                                for ( $x=0; $x<$result->num_rows;$x++){
                                                    if ($result->num_rows == 0) {
                                                        echo 'Your code for handling no results...';
                                                    } else {
                                                        while ($row = $result->fetch_assoc()) {
                                                            $scheduleid = $row["scheduleid"];
                                                            $title = $row["title"];
                                                            $docname = $row["fname"]; // Correctly using the alias 'docname'
