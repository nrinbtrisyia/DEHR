...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    if (isset($_POST["search"])) {
        ...
        $sqlmain = "SELECT * FROM appointment JOIN patient ON patient.patientID=appointment.patientID JOIN schedule ON schedule.scheduleid=appointment.scheduleid JOIN doctor ON doctor.doctorID = appointment.doctorID";
...
if ($current == "My patients") {
    $sqlmain = "SELECT * FROM appointment 
                ...
                WHERE doctor.doctorID = $userid 
                AND appointment.doctorID != 0"; 
...
?>
    ...
    <div style="display: flex; margin-top: 40px;">
        ...
        <div style="margin-left: 20px;"> <!-- Add margin here -->
            <!-- <button id="myBtn" class="login-btn btn-primary btn button-icon" onclick="openmodal()">Old Patient's Record</button> -->
                ...
                <tr>
                    <td colspan="4" style="padding-top:0px;width: 100%;" >
                        ...
                        <select name="showonly" id="showonly" class="box filter-container-items" style="width:90% ;height: 37px;margin: 0;" >
                            ...
                            $doctorResult = $database->query("SELECT doctorID, fname FROM doctor");
                            if ($doctorResult->num_rows > 0) {
                                while ($doctor = $doctorResult->fetch_assoc()) {
                                    echo "<option value='doctor_{$doctor['doctorID']}'>Dr. {$doctor['fname']}</option><br/>";
                                }
                            }
                            ?>
                            ...
                            <?php
                                ...
                                for ( $x=0; $x<$result->num_rows;$x++){
                                    ...
                                    echo '<tr>
										...
										&nbsp;
										<a onclick="generateReport('.$appID.')" class="non-style-link">
                                            <button class="btn-primary-soft btn button-icon btn-report" style="padding-left: 40px; padding-top: 12px; padding-bottom: 12px; margin-top: 10px;">
                                                <font class="tn-in-text">Generate Report</font>
                                            </button>
                                        </a>
